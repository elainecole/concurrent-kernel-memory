...
int *primes_computed; // a pointer to the array of integers, within which primes will be computed

struct mutex prime_lock;
struct mutex curr_pos_lock;
...
static void remove_primes(int prime_index, ulong thread_index){
	int i;
	int prime;
	prime = primes_computed[prime_index];
	if(prime == 0){
		return;
	}
...
static void compute_primes(ulong thread_index){
	...
	while(current_position < prime_bound){
		mutex_lock(&curr_pos_lock);
		i = current_position;
		current_position++;
		while(current_position < prime_bound && primes_computed[current_position] == 0){
			current_position++;
		}
		mutex_unlock(&curr_pos_lock);

		mutex_lock(&prime_lock);
		remove_primes(i, thread_index);
		mutex_unlock(&prime_lock);
	}
}
...
static int __init lab02_module_init(void) {
	...
	if(num_threads > 0 && upper_bound > 1){
		...
		primes_computed = (int*) kmalloc(prime_bound * sizeof(int),  GFP_KERNEL);
		...
		mutex_init(&curr_pos_lock);
		mutex_init(&prime_lock);
	...
	if(num_threads > 0 && upper_bound > 1){
		if(atomic_read(&all_threads_finish) == num_threads_created){
			...
			while(i < prime_bound){
				j = 0;
				while(j < 8){
					if(primes_computed[i] == 0){
						++num_composites;
					}
					else{
						printk(KERN_CONT "%d ", primes_computed[i]);
						++num_primes;
						++j;	
					}
					++i;
...
MODULE_DESCRIPTION("Concurrent Memory Use Module");
